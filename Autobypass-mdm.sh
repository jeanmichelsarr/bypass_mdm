...
options=("Autoypass on Recovery" "Reboot" "Disable Notification (SIP)" "Disable Notification (Recovery)" "Exit")
select opt in "${options[@]}"; do
	case $opt in
	"Autoypass on Recovery")
		echo -e "${GRN}Bypass on Recovery"
		if [ -d "/Volumes/macOS Base System - Data" ]; then
   			diskutil rename "macOS Base System - Data" "Data"
		fi
